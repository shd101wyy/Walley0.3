...
*/
var RUN_FIRST = '(def (list . args) args) (defmacro cond (arg . args) (if (eq? (car arg) (quote else)) (cons (quote begin) (cdr arg)) (if (null? args) (let (judge (car arg) body (cons (quote begin) (cdr arg))) (quasiquote (if (unquote judge) (unquote body) ()))) (let (rest (cons (quote cond) args) judge (car arg) body (cons (quote begin) (cdr arg))) (quasiquote (if (unquote judge) (unquote body) (unquote rest))))))) (set! + (let (o_+ + +_iter (lambda (result args) (cond ((null? args) result) (else (+_iter (o_+ result (car args)) (cdr args)))))) (lambda (. args) (cond ((null? args) (display #str{"ERROR: Function + invalid parameters. Please provide parameters"}#)) (else (+_iter (car args) (cdr args))))))) (set! - (let (o_- - -_iter (lambda (result args) (cond ((null? args) result) (else (-_iter (o_- result (car args)) (cdr args)))))) (lambda (. args) (cond ((null? args) (display #str{"ERROR: Function - invalid parameters. Please provide parameters"}#)) ((null? (cdr args)) (o_- 0 (car args))) (else (-_iter (car args) (cdr args))))))) (set! * (let (o_* * *_iter (lambda (result args) (cond ((null? args) result) (else (*_iter (o_* (car args) result) (cdr args)))))) (lambda (. args) (cond ((null? args) (display #str{"ERROR: Function * invalid parameters. Please provide parameters"}#)) (else (*_iter (car args) (cdr args))))))) (set! / (let (o_div / div_iter (lambda (result args) (cond ((null? args) result) (else (div_iter (o_div result (car args)) (cdr args)))))) (lambda (. args) (cond ((null? args) (display #str{"ERROR: Function / invalid parameters. Please provide parameters"}#)) ((null? (cdr args)) (o_div 1 (car args))) (else (div_iter (car args) (cdr args))))))) (def #t "true") (def #f ()) (def nil ()) (def (factorial n) (if (eq? n 0) 1 (* n (factorial (- n 1))))) (set! factorial (lambda (n) (def (factorial-acc n acc) (if (eq? n 0) acc (factorial-acc (- n 1) (* acc n)))) (factorial-acc n 1))) (defmacro and (. args) (if (null? args) true (let (judge (car args)) (quasiquote (if (unquote judge) (apply and (cdr (quote (unquote args)))) false))))) (defmacro or (. args) (if (null? args) false (let (judge (car args)) (quasiquote (if (unquote judge) true (apply or (cdr (quote (unquote args))))))))) (def (> arg0 arg1) (< arg1 arg0)) (def (<= arg0 arg1) (or (< arg0 arg1) (eq? arg0 arg1))) (def (>= arg0 arg1) (or (> arg0 arg1) (eq? arg0 arg1))) (set! < (let (old-< < <-iter (lambda (args cur) (if (null? args) true (if (old-< cur (car args)) (<-iter (cdr args) (car args)) false)))) (lambda (. args) (if (null? args) (display #str{"Please provide arguments for <"}#) (<-iter (cdr args) (car args)))))) (set! > (let (old-> > >-iter (lambda (args cur) (if (null? args) true (if (old-> cur (car args)) (>-iter (cdr args) (car args)) false)))) (lambda (. args) (if (null? args) (display #str{"Please provide arguments for >"}#) (>-iter (cdr args) (car args)))))) (set! eq? (let (old-eq? eq? eq?-iter (lambda (args cur) (if (null? args) true (if (old-eq? cur (car args)) (eq?-iter (cdr args) (car args)) false)))) (lambda (. args) (if (null? args) (display #str{"Please provide arguments for eq?"}#) (eq?-iter (cdr args) (car args)))))) (set! <= (let (old-<= <= <=-iter (lambda (args cur) (if (null? args) true (if (old-<= cur (car args)) (<=-iter (cdr args) (car args)) false)))) (lambda (. args) (if (null? args) (display #str{"Please provide arguments for <="}#) (<=-iter (cdr args) (car args)))))) (set! >= (let (old->= >= >=-iter (lambda (args cur) (if (null? args) true (if (old->= cur (car args)) (>=-iter (cdr args) (car args)) false)))) (lambda (. args) (if (null? args) (display #str{"Please provide arguments for >="}#) (>=-iter (cdr args) (car args)))))) (def (not arg0) (if arg0 false true)) (def (pair? arg) (eq? (typeof arg) (quote list))) (def (list? arg) pair?) (def (string? arg) (eq? (typeof arg) (quote string))) (def (vector? arg) (eq? (typeof arg) (quote vector))) (def (dictionary? arg) (eq? (typeof arg) (quote dictionary))) (def (undefined? arg) (eq? arg (quote undefined))) (def (procedure? arg) (eq? (typeof arg) (quote procedure))) (def (function? arg) procedure?) (def (macro? arg) (eq? (typeof arg) (quote macro))) (def ** (math :pow)) (def ^ (math :pow)) (assoc! math :log (lambda (x y) (/ ((math :loge) y) ((math :loge) x)))) (assoc! math :sec (lambda (x) (/ 1 ((math :cos) x)))) (assoc! math :csc (lambda (x) (/ 1 ((math :sin) x)))) (assoc! math :cot (lambda (x) (/ 1 ((math :tan) x)))) (def (% n0 n1) (let (result (->int (/ n0 n1))) (- n0 (* result n1)))) (def (append a b) (if (null? a) b (cons (car a) (append (cdr a) b)))) (def (reverse l) (def (reverse-iter result l) (if (null? l) result (reverse-iter (cons (car l) result) (cdr l)))) (reverse-iter () l)) (defmacro defn (fn_name args . body) (list (quote def) fn_name (cons (quote lambda) (cons args body)))) (defmacro comment (. args) ()) (def (map func . args) (def length (len (car args))) (def (get-args-list args i) (if (null? args) () (cons (ref (car args) i) (get-args-list (cdr args) i)))) (def (map-iter func args i) (cond ((eq? i length) ()) (else (cons (apply func (get-args-list args i)) (map-iter func args (+ i 1)))))) (map-iter func args 0)) (def (str . args) (def (str-iter result args) (if (null? args) result (str-iter (+ result (->str (car args))) (cdr args)))) (str-iter "" args)) (def (->float num) (* num 1.0000000000)) (def (->int num) (cond ((eq? num 0) 0) ((> num 0) ((math :floor) num)) (else ((math :ceil) num)))) (def (diff func diff-at-point :error 0.0000010000) (def (diff_ error) (/ (- (func diff-at-point) (func (- diff-at-point error))) error)) (diff_ error)) (def (integral func a b :dx 0.0100000000) (def (sum term a next b) (if (> a b) 0 (+ (term a) (sum term (next a) next b)))) (def (add-dx x) (+ x dx)) (* (sum func (+ a (/ dx 2.0000000000)) add-dx b) dx)) (def (set-car x value) (cons value (cdr x))) (def (set-cdr x value) (cons (car x) value)) (set! undefined? (lambda (var-name) (__undefined?__ var-name (get-env)))) (def (make-gensym) (let (count 0 step 1 set-name (lambda () (set! count (+ count step)) (let (var-name (str "t_" count)) (if (undefined? var-name) var-name (set-name))))) set-name)) (def gensym (make-gensym)) (defmacro load (file-name) (quasiquote (eval (cons (quote begin) (read (read-file (unquote file-name))))))) (defmacro for ((i l) . body) (quasiquote (let (f (lambda (l) (if (null? l) (quote undefined) (let ((unquote i) (car l)) (unquote-splice body) (f (cdr l)))))) (f (unquote l)))))'
